...
public class Ataque {
	...
	private Position[] calculatePositions(Position[] positions){
		double height = Math.max(Constants.FIELD_LENGTH/3, 2*Math.min(Math.abs(Constants.BOTTOM_GOAL_CENTER.getY()-sp.ballPosition().getY()), Math.abs(Constants.TOP_GOAL_CENTER.getY()-sp.ballPosition().getY())));
		double width = Math.max(Constants.FIELD_WIDTH*2/3, 2*Math.min(Math.abs(Constants.BOTTOM_LEFT_CORNER.getX()-sp.ballPosition().getX()), Math.abs(Constants.BOTTOM_RIGHT_CORNER.getX()-sp.ballPosition().getX())));
		Position center = new Position(sp.ballPosition().getX(), sp.ballPosition().getY());
		if(center.getX() + width/2 > Constants.BOTTOM_RIGHT_CORNER.getX()){
			center = center.movePosition(-(center.getX() + width/2 - Constants.BOTTOM_RIGHT_CORNER.getX()), 0);
		}
		if(center.getX() - width/2 < Constants.BOTTOM_LEFT_CORNER.getX()){
			center = center.movePosition(Constants.BOTTOM_LEFT_CORNER.getX() - (center.getX()-width/2), 0);
		}
		if(center.getY() + height/2 > Constants.TOP_GOAL_CENTER.getY()){
			center = center.movePosition(0, -(center.getY() + height/2 - Constants.TOP_GOAL_CENTER.getY()));
		}
		if(center.getY() - height/2 < Constants.BOTTOM_GOAL_CENTER.getY()){
			center = center.movePosition(0, Constants.BOTTOM_GOAL_CENTER.getY() - (center.getY() - height/2));
		}
		Position[] newPositions = new Position[11];
		for (int i = 0; i < newPositions.length; i++) {
			newPositions[i] = new Position(positions[i].getX()*width/Constants.FIELD_WIDTH + center.getX(), positions[i].getY()*height/Constants.FIELD_LENGTH + center.getY());
		}
		return newPositions;
	}
}
