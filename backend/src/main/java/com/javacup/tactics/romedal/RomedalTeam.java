package com.javacup.tactics.romedal;
...
import com.javacup.tactics.romedal.util.Analyze;
import com.javacup.tactics.romedal.util.Analyze.Direction;
import com.javacup.tactics.romedal.util.Analyze.MyAceleration;
import com.javacup.tactics.romedal.util.Ball;
import com.javacup.tactics.romedal.util.BallDestiny;
import com.javacup.tactics.romedal.util.HitBallDetail;
import com.javacup.tactics.romedal.util.RMConstants;
import com.javacup.tactics.romedal.util.ShootingTrajectory;
...
public class RomedalTeam implements Tactic, Alineaciones, RMConstants {
	...
	private int obtenerTrayectoriaCalculada(final LinkedList<Ball> trayectoriaBalon, final Ball balon, final double potencia, final double angleXY, final double angleZ, final Position pFinal, final boolean completa, final int idxReceptor,
		...
		final boolean centro = sp.ballPosition().distance(Constants.FIELD_CENTER) == 0;
		for (int step = 1; step <= maxPasos; step++) {
			final double time = step / 60d;
			final double dX = trajectory.getX(time) * Constants.TRAJECTORY_VELOCITY_AMPLIFIER;
			final double dY = trajectory.getY(time) * Constants.TRAJECTORY_VELOCITY_AMPLIFIER * 2;
				...
				if (iteracionesAlcance == -1) {
					if (idxReceptor == -1 && !trayectoriaBalon.isEmpty()) {
						if (angleZ > 0) {
							...
							final boolean antAdentro = balonAnterior.getAltura() > Constants.GOAL_HEIGHT && balonAnterior.getPosition().getY() <= META_Y;
							final boolean actAfuera = nuevoBalon.getAltura() < Constants.GOAL_HEIGHT && nuevoBalon.getPosition().getY() > META_Y;
							if (antAdentro && actAfuera) {
								...
								if (alturaMeta <= Constants.GOAL_HEIGHT && (nuevoBalon.getAltura() - (balonAnterior.getAltura() - b0.getAltura()) <= Constants.GOAL_HEIGHT || centro)) {
									trayectoriaBalon.add(nuevoBalon);
									return step;
								}
							}
						} else if (nuevoBalon.getPosition().getY() >= Constants.FIELD_LENGTH / 2 && Math.abs(nuevoBalon.getPosition().getX()) <= Constants.GOAL_WIDTH) {
							trayectoriaBalon.add(nuevoBalon);
							return step;
						}
					} else if (iteracionesAlcance == -1 && distanciaAPunto <= datosControl[0] && (angleZ == 0 || (dY <= datosControl[1]))) {
						iteracionesAlcance = Math.max(step, Constants.KICK_COOLDOWN_ITERATIONS);
						if (!completa) {
							trayectoriaBalon.add(nuevoBalon);
	...
	private void ubicarPortero(BallDestiny destino) {
		if (sp.iteration() > 1) {
			...
			if (!sp.isStarts() && (sp.ballPosition().getY() < 0 || !analisys.enAreaChica(goalkeeperPos, false))) {
				...
				} else if (sp.ballPosition().distance(goalkeeperPos) > Constants.BALL_CONTROL_DISTANCE) {
	...
	private Position mejorAnguloPosArquero() {
		final double distDer = Constants.BOTTOM_GOAL_RIGHT_POST.distance(sp.ballPosition()) / 3;
		final double distIzq = Constants.BOTTOM_GOAL_LEFT_POST.distance(sp.ballPosition()) / 3;
		...
		if (minDist <= Constants.LARGE_AREA_WIDTH && distRival <= Constants.BALL_CONTROL_DISTANCE) {
			final double anguloPoste = Constants.BOTTOM_GOAL_LEFT_POST.angle(sp.ballPosition());
			final Position posRet = Position.intersection(sp.ballPosition(), Constants.BOTTOM_GOAL_CENTER, Constants.BOTTOM_GOAL_RIGHT_POST, Constants.BOTTOM_GOAL_LEFT_POST.moveAngle(anguloPoste, distIzq));
			if (posRet != null) {
				double angle = sp.myPlayers()[idxMyGoalkeeper].angle(posRet);
				return sp.myPlayers()[idxMyGoalkeeper].moveAngle(angle, 0.3);
			}
		}
		final double angulo = Constants.BOTTOM_GOAL_CENTER.angle(sp.ballPosition());
		return Constants.BOTTOM_GOAL_CENTER.moveAngle(angulo, (Constants.GOAL_WIDTH - Constants.GOALKEEPER_BALL_CONTROL_DISTANCE) / 2);
	}
	...
	private void recuperarBalon(final BallDestiny destino) {
		...
		if (!laTengo) {
			if (!analisys.getRealTrayectory().isEmpty() && (analisys.getRealTrayectory().getLast().getPosition().getY() >= 52.5 || sp.ballPosition().equals(metaArribaDerecha) || sp.ballPosition().equals(metaArribaIzquierda))) {
				final int idx = Constants.TOP_GOAL_CENTER.nearestIndex(sp.myPlayers());
				recuperando.add(idx);
		...
		if (!trayectoriaBalon.contains(balonDestinoPase)) {
			...
			for (final Ball balon : trayectoriaBalon) {
				if (balon.getPosition().isInsideGameField(0)) {
					...
					if (distancia <= Constants.BALL_CONTROL_DISTANCE && balon.getAltura() <= Constants.BALL_CONTROL_HEIGHT && balon.getIteracion() >= sp.iterationsToKick()[destinoPase]) {
						alcanzaBalon = true;
						opcion = balon;
	...
	private boolean puedoGolpear() {
		if (sp.canKick().length > 0 && sp.ballPosition().distance(Constants.FIELD_CENTER) == 0) {
			return true;
		}
		if (sp.canKick().length == 0 || sp.isStarts() || iterSaco <= Constants.MAX_SET_PIECE_ITERATIONS * 3.5 / 5) {
			iterSaco++;
			double maxIter = ((Constants.MAX_SET_PIECE_ITERATIONS * 4 / 5) - iterSaco) * 0.45;
			...
			if (sp.ballPosition().distance(Constants.TOP_RIGHT_CORNER) == 0 || sp.ballPosition().distance(Constants.TOP_LEFT_CORNER) == 0) {
				final int j = Constants.TOP_PENALTY_SPOT.nearestIndex(sp.myPlayers(), i, receptor);
				comandos.add(analisys.moveTo(j, Constants.TOP_PENALTY_SPOT));
				recuperando.add(j);
			}
		}
		if (sp.canKick().length > 0) {
			for (int i : sp.canKick()) {
				if (sp.ballPosition().distance(sp.myPlayers()[i]) <= (i == idxMyGoalkeeper && analisys.enAreaGrande(sp.ballPosition(), false) ? Constants.GOALKEEPER_BALL_CONTROL_DISTANCE : Constants.BALL_CONTROL_DISTANCE)) {
					return true;
				}
			}
		}
	...
	private boolean hacerPase(final int minIterDif, boolean autopase, boolean toBack) {
		final int[] receptores = Constants.TOP_GOAL_CENTER.nearestIndexes(sp.myPlayers(), idxMyGoalkeeper, ultimoPasador);
				...
				if (!analisys.isOffSide(idxReceptor, false)) {
					...
					if (dest != null) {
						...
						if (idxReceptor == idxMyGoalkeeper) {
							...
							Position pFinal = Position.intersection(sp.ballPosition(), sp.ballPosition().moveAngle(ang, 100), Constants.BOTTOM_GOAL_CENTER.movePosition(-Constants.GOAL_WIDTH, 0),
									Constants.BOTTOM_GOAL_CENTER.movePosition(Constants.GOAL_WIDTH, 0));
							if (pFinal != null && Math.abs(pFinal.getX()) <= Constants.GOAL_WIDTH) {
								descartar = true;
							}
						}
						if (!descartar) {
	...
	private void despejarBalon() {
		if (sp.ballPosition().getY() > 35 || ultimoPasador == idxMyGoalkeeper) {
			comandos.add(new CommandHitBall(ultimoPasador, Constants.TOP_GOAL_CENTER, 1, ultimoPasador == idxMyGoalkeeper));
			return;
		}
		if (!sp.isStarts() && sp.ballPosition().getY() <= 0) {
			...
			if (rival.distance(sp.myPlayers()[ultimoPasador]) > 1.5) {
				comandos.add(new CommandHitBall(ultimoPasador, Constants.TOP_PENALTY_SPOT, 1, 60));
				...
				if (ultimoPasador == idxMyGoalkeeper || (sp.iteration() - iterDespeje == 1 && sp.iteration() > 1 && sp.ballPosition().distance(analisys.getPreviousBall().getPosition()) < 0.1)) {
					comandos.add(new CommandHitBall(ultimoPasador, Constants.TOP_PENALTY_SPOT, 1, true));
				} else {
			...
			if (sp.rivalCanKick().length > 0) {
				comandos.add(analisys.moveTo(ultimoPasador, Constants.BOTTOM_PENALTY_SPOT));
				marcando.add(ultimoPasador);
			} else if (!hacerDisparo(false, 20, 45)) {
				comandos.add(new CommandHitBall(ultimoPasador, Constants.TOP_PENALTY_SPOT, 1, Constants.MAX_VERTICAL_ANGLE));
			}
		} else if (!hacerDisparo(false, 20, 45)) {
			comandos.add(new CommandHitBall(ultimoPasador, Constants.TOP_PENALTY_SPOT, 1, Constants.MAX_VERTICAL_ANGLE));
		}
	...
	private void golpearBalon() {
		...
		if (puedoGolpear()) {
			iterSaco = 0;
			double distanceToGoal = sp.ballPosition().distance(Constants.TOP_GOAL_CENTER);
	...
	private void setAlineacion() {
		...
		if (sp.ballPosition().getY() < -45 || (sp.isRivalStarts() && sp.ballPosition().getY() < 0)) {
			...
			final double y = Math.max(0, refY + Constants.TOP_GOAL_CENTER.getY() - 1);
		...
		for (final Command c : comandos) {
			if (c instanceof CommandMoveTo) {
				...
				if (sp.myPlayers()[cm.getPlayerIndex()].distance(cm.getMoveTo()) < Constants.MIN_SPEED) {
					listaBorrar.add(cm);
				}
			}
		}
		...
		for (int i = 1; i < alineacion.length; i++) {
			if (!excluir.contains(i)) {
				...
				if (laTengo) {
					double angulo = 0;
					if (pos.getX() < 0) {
						angulo = pos.angle(Constants.TOP_GOAL_LEFT_POST);
					} else {
						angulo = pos.angle(Constants.TOP_GOAL_RIGHT_POST);
					}
	...
	private BallDestiny getBestLocated(final boolean analizarPortero, final boolean soloMios) {
		...
		if (analizarPortero && myDestiny != null && rivalDestiny != null) {
			...
			if (((area && alt <= Constants.GOAL_HEIGHT && myDestiny.compareTo(rivalDestiny) >= 0) || (!area && alt < Constants.BALL_CONTROL_HEIGHT && myDestiny.compareTo(rivalDestiny) > 0))) {
				return myDestiny;
			}
		}
	...
	private BallDestiny getPlayerDestiny(final LinkedList<Ball> trajectory, final int idx, final boolean rival, final int maxIter, final boolean realTrajectory) {
		if (trajectory.size() > 1) {
			...
			final double maxHeight = analisys.isGoalkeeper(idx, rival) ? Constants.GOAL_HEIGHT : Constants.BALL_CONTROL_HEIGHT;

			int iterControl = rival ? sp.rivalIterationsToKick()[idx] : (realTrajectory || idx != ultimoPasador ? sp.iterationsToKick()[idx] : Constants.KICK_COOLDOWN_ITERATIONS);

			MyAceleration aceleration = analisys.getAceleration(idx, rival);

			double speed = Constants.getSpeed(rival ? sp.getRivalPlayerSpeed(idx) : sp.getMyPlayerPower(idx));
	...
	private HitBallDetail getOpcionPase(final int idxPasador, final int idxReceptor, final int minIterDif, boolean toBack) {
		...
		final int pasoIni = autoPase ? Constants.KICK_COOLDOWN_ITERATIONS : 0;
		...
		double speed = Constants.getSpeed(sp.getMyPlayerPower(idxReceptor));
		...
		for (final int ang : angulosPosibles.keySet()) {
			...
			for (int paso = pasoIni; paso <= MAX_ITER; paso += 5) {
				...
				if ((autoPase || (distancia >= distanciaMinima))) {
					...
					if (!autoPase) {
						infoTrayectoria = getPassOption(idxPasador, banda, distancia + (Constants.BALL_CONTROL_DISTANCE / 2), Constants.BALL_CONTROL_HEIGHT, paso);
					...
					if (autoPase || comando == null) {
						...
						boolean possible = pFinal.distance(sp.rivalPlayers()[closed]) > distance + Constants.BALL_CONTROL_DISTANCE;
						if (possible) {
							infoTrayectoria = getPassOption(idxPasador, banda, distancia + Constants.BALL_CONTROL_DISTANCE, 0, paso);
							comando = disparoAlPunto(infoTrayectoria, idxPasador, pFinal, idxReceptor, paso, minIterDif);
						}
					}
					if (comando != null) {
	...
	private HitBallDetail disparoAlPunto(final ShootingTrajectory shotInfo, final int idxPateador, final Position pFinal, final int idxReceptor, int maxIteraciones, double minIterDif) {
		if (shotInfo != null) {
			...
			final int minIter = idxPateador == idxReceptor || idxReceptor == -1 ? Constants.KICK_COOLDOWN_ITERATIONS : sp.iterationsToKick()[idxReceptor];
	...
	private ShootingTrajectory getPassOption(final int idxPlayer, final boolean banda, final double distancia, final double altFinal, final int minIteraciones) {
		...
		double maxPower = Constants.getKickVelocity(sp.getMyPlayerPower(idxPlayer));
		double factorReduccion = sp.getMyPlayerEnergy(idxPlayer);
		factorReduccion *= Constants.ENERGY_SHOT_FACTOR;
	...
	private HitBallDetail mejorDisparoAlArco(final int idxPateador, final boolean obstaculo, final double minAngZ, final double maxAngZ) {
		...
		final double velocidadRemate = Constants.getKickVelocity(pateador.getPower());
		final double maxX = (Constants.GOAL_WIDTH / 2) - Constants.BALL_RADIUS;
		final LinkedList<Position> opciones = new LinkedList<Position>();
		final Position pInicial = Constants.TOP_GOAL_CENTER;
		...
		for (final Position position : opciones) {
			...
			for (double fuerza = 1; fuerza > (maxAngZ > 0 ? 0.8 : 0.95); fuerza -= 0.05d) {
				...
				for (double angZ = minAngZ; angZ <= maxAngZ; angZ += 0.5) {
					final double angRadZ = Math.max(0, Math.min(angZ, Constants.MAX_VERTICAL_ANGLE)) * (TO_RAD);
					final LinkedList<Ball> trayectoriaBalon = new LinkedList<Ball>();
