...
public class Analyze {
	...
	public void setRealTrayectory() {
		...
		while (insideField) {
			...
			if (!insideField || (p.getY() == yPrev && p.getX() == xPrev)) {
				if (!insideField && Math.abs(p.getY()) > Constants.FIELD_LENGTH / 2 && Math.abs(p.getX()) <= Constants.GOAL_WIDTH / 2) {
		...
		for (double strength = 1; strength >= 0.2; strength -= 0.005d) {
			...
			if (!autopass) {
				for (double angZ = RMConstants.MAX_ANGLE; angZ >= RMConstants.MIN_ANGLE; angZ -= RMConstants.DELTA_ANGLE) {
					...
					if (yMax > Constants.BALL_CONTROL_HEIGHT) {
	...
	private ShootingTrajectory setTrayectory(final double shotPower, final double angRadZ, boolean autopass, boolean banda) {
		...
		final double altControl = paseRastrero ? 0 : Constants.BALL_CONTROL_HEIGHT;

		final int iteracionesControl = Constants.KICK_COOLDOWN_ITERATIONS;
		...
		for (int step = 1; step <= (autopass ? iteracionesControl : 200); step++) {
			...
			double dX = airTrajectory.getX(time) * Constants.TRAJECTORY_VELOCITY_AMPLIFIER;
			final double dY = airTrajectory.getY(time) * Constants.TRAJECTORY_VELOCITY_AMPLIFIER * 2;
			final double y = dX;
			...
			} else if (angRadZ == 0 && step >= iteracionesControl && dX - dxAnt <= Constants.BALL_CONTROL_DISTANCE) {
				trajectory.setDistancia(dX);
				trajectory.setIteraciones(step);
				return trajectory;
			}
	...
	public boolean idBanda() {
		return Math.abs(sp.ballPosition().getX()) >= Constants.FIELD_WIDTH / 2 && Math.abs(sp.ballPosition().getX()) < Constants.FIELD_LENGTH / 2;
	}
	...
	public double getDistance(Position playerPos, MyAceleration acel, int iter, double angle, double speed, double energy, Direction prevDir) {
		...
		if (prevDir.x != newDir.x)
			aceleracionX = Constants.MIN_ACCELERATION_X;
		else {
			aceleracionX += Constants.ACCELERATION_INCREMENT;
			if (aceleracionX > 1)
				aceleracionX = 1;
		}

		if (prevDir.y != newDir.y)
			aceleracionY = Constants.MIN_ACCELERATION_Y;
		else {
			aceleracionY += Constants.ACCELERATION_INCREMENT;
			if (aceleracionY > 1)
				aceleracionY = 1;
		}
		...
		for (int i = 0; i < iter; i++) {
			...
			energy -= Constants.ENERGY_RECOVERY_RATE;

			energy = (energy < Constants.MIN_ENERGY) ? Constants.MIN_ENERGY : energy;
	...
	public double getPower(int idxPlayer, double potencia) {
		...
		double max = Constants.getKickVelocity(factor);

		if (energy < 0.8) {
			return potencia * energy * Constants.ENERGY_SHOT_FACTOR / max;
		}
		return potencia / max;
	}
	...
	public double[] getDatosControl(final int idxJugador, final Position p, final boolean esRival) {
		if ((idxJugador == -1 || isGoalkeeper(idxJugador, esRival)) && enAreaGrande(p, esRival)) {
			return new double[] { Constants.GOALKEEPER_BALL_CONTROL_DISTANCE, Constants.GOAL_HEIGHT };
		}
		return new double[] { Constants.BALL_CONTROL_DISTANCE, Constants.BALL_CONTROL_HEIGHT };
	}

	public boolean enAreaGrande(final Position pos, final boolean rival) {
		final double minY = ((Constants.FIELD_LENGTH / 2) - Constants.LARGE_AREA_WIDTH);
		final boolean yEnArea = rival ? pos.getY() >= minY : pos.getY() < -minY;
		if (Math.abs(pos.getX()) <= Constants.LARGE_AREA_LENGTH / 2 && yEnArea) {
			return true;
		}
		return false;
	}
	...
	public boolean enAreaChica(final Position pos, final boolean rival) {
		final double minY = ((Constants.FIELD_LENGTH / 2) - Constants.SMALL_AREA_WIDTH);
		final boolean yEnArea = rival ? pos.getY() >= minY : pos.getY() < -minY;
		if (Math.abs(pos.getX()) <= Constants.SMALL_AREA_LENGTH / 2 && yEnArea) {
			return true;
		}
		return false;
	}
	...
	public CommandMoveTo moveTo(int idx, Position destiny) {
		...
		double vel = Constants.getSpeed(sp.getMyPlayerSpeed(idx)) * sp.getMyPlayerEnergy(idx) * aceleracion;
