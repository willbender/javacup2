...
public class Team2313 implements Tactic {
	...
	public List<Command> execute(final GameSituations sp) {
		...
		if (sp.iteration() > 0) {
			...
			final int rivalMasPeligro = Constants.BOTTOM_GOAL_CENTER.nearestIndex(sp.rivalPlayers());
			...
			for (final int i : sp.canKick()) {
				final CommandHitBall cg = mejorDisparoAlArco(i, 10, Constants.MAX_VERTICAL_ANGLE, sp);
				...
				final double distanciaArco = posKick.distance(Constants.TOP_GOAL_CENTER);
				if (sp.isStarts() && iteracionesSaque >= Constants.MAX_SET_PIECE_ITERATIONS / 2) {
					if (cg != null && distanciaArco > Constants.LARGE_AREA_WIDTH) {
						com.add(cg);
					} else {
						if (sp.ballPosition().distance(Constants.TOP_GOAL_CENTER) > 40) {
							Position posDisparo = Constants.TOP_GOAL_CENTER;
							com.add(new CommandHitBall(i, posDisparo, 1.0, Constants.VERTICAL_ANGLE));
						} else {
							com.add(new CommandHitBall(i, Helper.obtenerPuntoMejorDisparoArco(sp), 1.0,
									Constants.VERTICAL_ANGLE * r.nextDouble()));
						}
					}
					iteracionesSaque = 0;
				}
				if (!sp.isStarts()) {
					if (cg != null && distanciaArco > Constants.LARGE_AREA_WIDTH
							&& distanciaArco < Constants.FIELD_LENGTH / 2) {
						com.add(cg);
					} else {
						if (sp.ballPosition().distance(Constants.TOP_GOAL_CENTER) > 40) {
							Position posDisparo = Constants.TOP_GOAL_CENTER;
							double anguloDisparo = Constants.VERTICAL_ANGLE;
							...
							if (posMiMarca != null && posMiMarca.distance(miPosicion) < 1) {
								...
								if (!pase) {
									posDisparo = Constants.TOP_GOAL_CENTER;
									anguloDisparo = Constants.MAX_VERTICAL_ANGLE;
								}
					...
					if (sp.myPlayers()[i].distance(Constants.TOP_GOAL_CENTER) < Constants.SMALL_AREA_LENGTH) {
						com.add(new CommandHitBall(i, Constants.TOP_GOAL_CENTER, 0.5, false));
					}
				}
			}
			...
			if (sp.isStarts()) {
				...
				if (iteracionesSaque < Constants.MAX_SET_PIECE_ITERATIONS / 2) {
					iteracionesSaque++;
				}
			}
	...
	private CommandHitBall mejorDisparoAlArco(final int idxPateador, final double minAngZ, final double maxAngZ,
		...
		final double maxX = (Constants.GOAL_WIDTH / 2);
		...
		for (final Position position : opciones) {
			...
			for (double fuerza = 1; fuerza > 0.6; fuerza -= 0.1d) {
				...
				for (double angZ = minAngZ; angZ <= maxAngZ; angZ += 0.5) {
					...
					final double angRadZ = Math.max(0, Math.min(angZ, Constants.MAX_VERTICAL_ANGLE)) * (RAD);
					...
					if (iteraciones >= 0) {
						...
						int iterRival = (int) Math.ceil(distRivAlPunto
								/ Constants.getSpeed(sp.rivalPlayersDetail()[idxRival].getSpeed()));
						if (destino != null) {
							idxRival = destino.getIdxJugador();
	...
	private int obtenerTrayectoriaCalculada(final LinkedList<Ball> trayectoriaBalon, final Ball balon,
		...
		final double[] datosControl = new double[] { Constants.GOALKEEPER_BALL_CONTROL_DISTANCE, Constants.GOAL_HEIGHT };

		// final int iteracionesControl = idxReceptor == -1 ?
		// sp.rivalIterationsToKick()[idxPortRival] :
		// Constants.KICK_COOLDOWN_ITERATIONS;
				...
				if (iteracionesAlcance == -1) {
					...
					if (idxReceptor == -1 && !trayectoriaBalon.isEmpty()) {
						if (angleZ > 0) {
							...
							final boolean antAdentro = balonAnterior.getAltura() > Constants.GOAL_HEIGHT
									&& balonAnterior.getPosition().getY() <= (Constants.FIELD_LENGTH / 2);
							final boolean actAfuera = nuevoBalon.getAltura() < Constants.GOAL_HEIGHT
									&& nuevoBalon.getPosition().getY() > (Constants.FIELD_LENGTH / 2);
							if (antAdentro && actAfuera) {
								...
								final double nuevoX = deltaX
										- ((Constants.FIELD_LENGTH / 2) - balonAnterior.getPosition().getY());
								final double alturaMeta = nuevoBalon.getAltura() + (Math.tan(teta) * nuevoX);
								// si la altura en linea de meta es aceptable
								if (alturaMeta <= Constants.GOAL_HEIGHT
										&& (nuevoBalon.getAltura() - (balonAnterior.getAltura() - b0.getAltura()) <= Constants.GOAL_HEIGHT || centro)) {
									trayectoriaBalon.add(nuevoBalon);
									return step;
								}
							}
						} else if (nuevoBalon.getPosition().getY() >= Constants.FIELD_LENGTH / 2
								&& Math.abs(nuevoBalon.getPosition().getX()) <= Constants.GOAL_WIDTH) {
							trayectoriaBalon.add(nuevoBalon);
							return step;
						}
					}
